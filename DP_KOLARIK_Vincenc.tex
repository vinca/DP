...
\acknowledgements{Thanks to my family and especially to my girlfriend, Tamara, for all their patience and care. Thanks to my thesis supervisor, Robert, for everything he taught me.}

\abstractEN{The thesis is engaged in evolvability of applications build using microservice architecture. It applies the rules defined by the Normalized Systems theory and discusses its impacts. The thesis thoroughly analyses the domain of microservice architecture and identifies the key architectural aspects. The aspects are examined using the Normalized Systems theory and results in a~set of guidelines to achieve evolvability of the microservice applications. The viability of proposed guidelines is then discussed.}
\keywordsEN{evolvability, microservice architecture, microservices, normalized systems theory, software architecture}

\abstractCS{Práce se zabývá schopností evolvability aplikací postavených na architektuře mikroslužeb. Aplikuje pravidla definovaná teorií Normalizovaných systémů a~diskutuje jejich dopad. Práce zevrubně zkoumá doménu architektury\break mikroslužeb a identifikuje její klíčové architektonické aspekty. Aspekty jsou dále zkoumány pomocí teorie Normalizovaných systémů. Výsledkem je soubor pravidel pro dosažení evolvability aplikací využívajících mikroslužeb. Vhodnost navrhovaných pravidel je poté diskutována.}
\keywordsCS{architektura mikroslužeb, evolvabilita, mikroslužby, softwarová architektura, teorie normalizovaných systémů\clearpage}
...
\newacronym{ACD}{ACD}{atomicity, consistency, durability}
...
\newacronym{DRY}{DRY}{do not repeat yourself}
...
\newacronym{JWT}{JST}{JSON web token}
...
Successful Internet applications have tenths of \acrshort{GUI}s: web browser \acrfull{FE}, desktop apps, mobile apps, watch apps and desktop widget. Due to the heterogeneity of the human-facing interfaces, they are out of scope of this thesis.
...
\begin{enumerate}
    ...
    \item Focus on the internal architecture of the \acrfull{BE}.
\end{enumerate}
...
The four views are not fully independent or orthogonal. Elements of one view must relate to elements in other views, otherwise, the model has some inconsistencies (e.g., non deployed code artifacts, not utilized or inaccessible \acrshort{VM}s). Directions of those relationships --- \textit{mappings} --- between the views are shown in fig. \ref{fig:arch-41-view} using the black arrows.
...
\begin{itemize}
    \item \textbf{The network is reliable} has a whole \fullref{sec:resilience} dedicated to dealing with this phenomenon.
    \item \textbf{Latency is zero}, \textbf{transport cost is zero}, and \textbf{bandwidth is infinite} have a great impact on applications performance. The ways how to mitigate this are discussed in \fullref{sec:inter-service-communication}.
    \item \textbf{Topology does not change}, and \textbf{the network is homogeneous} is mitigated using e.g. service discovery, discussed in \fullref{sec:operations}
    \item \textbf{The network is secure}, and \textbf{there is one administrator} belong to the application operations domain, therefore are out of scope of this analysis.
\end{itemize}
...
\begin{table}[!hb]
\centering
\caption{Interaction Model to Interaction Style Matrix}
...
Since both of the model dimensions must have a value, actual communication is modelled by the carthesian product of those two dimensions --- an \textit{interaction style} (see tab. \ref{tab:styles-models}).
...
\label{sec:selected_aspects}
\begin{itemize}[leftmargin=35pt,align=center]
    \item[\faCheck] \textbf{Microservice scope} --- a crucial question which is now answered only with a very vague guidelines. Will be discussed separately.
    \item[\faCheck] \textbf{Inner vs. outer architecture} --- often neglected, yet important topic, closely related to \textit{separation of concerns}. Will be discussed separately.
    \item[\faCheck] \textbf{Polyglotism, technological diversity} --- one of the most hyped benefits of \acrshort{MSA} and potential source of complexity. Will be discussed separately.
    \item[\faRemove] \textbf{Trade-offs of distributed system} --- although this is a set of important topics, it is actually only a list of essential problems with distributed architecture. Solutions to those problems are discussed in \fullref{sec:fallacies}. Therefore this topic won't be discussed separately.
    \item[\faRemove] \textbf{Interaction models and styles} --- only a taxonomy, does not represent real patterns or problems. The real communication complexity is discussed in \fullref{sec:appl:trans}. 
    \item[\faCheck] \textbf{Service contracts} --- will be discussed in \fullref{sec:appl:inout} and \fullref{sec:ccc}.
    \item[\faCheck] \textbf{N-phase commits} --- will be discussed in \fullref{sec:appl:trans}.
    \item[\faCheck] \textbf{Saga pattern} --- will be discussed in \fullref{sec:appl:trans}.
    \item[\faCheck] \textbf{Persistence} --- will be discussed separately.
    \item[\faCheck] \textbf{External APIs} --- will be discussed separately.
    \item[\faRemove] \textbf{Deployment and Operations} --- operations fall out of scope of this thesis; the interesting topics related to this subdomain are listed in \fullref{sec:ccc}.
    \item[\faRemove] \textbf{Scalability / Elasticity} --- scalability is a matter of operations, therefore does not fall into the scope of this thesis. The only requirements to the microservice is to be transactional and stateless (both are discussed in in \fullref{sec:appl:trans}).
    ...
    \item[\faCheck] \textbf{Cross-cutting concerns} --- will be discussed separately.
\end{itemize}
...
There is also a definition of microservice by Sam Newman: \textit{A microservice should be rewritten by a team of developers in two weeks.}~\cite{ms-building-ms} Implication of this statement is that the complexity of one \acrshort{MS} is bounded (to the two weeks of work of one team). Considering that, microservice itself as a Normalized System does not bring any value.
...
\footnotetext{This is probably the point where a veteran software engineer can't contain his indignation anymore, and stops reading while yelling: \textit{This is ridiculous, spinning up a Docker container just to have a configured cron running somewhere! What a waste!} If you are one of those, please, hold on. This suboptimal use of system resources will hopefully result in infinite evolvability.}
...
Normalized Systems consider persistence a cross-cutting concern. Due to the \acrshort{MSA} pattern \textit{one DB per service}, it is the private matter of the single service. None of the principles of the \acrshort{NS} seem to contradict that assumption. Therefore, the definition of inner and outer architecture considered compliant with \acrshort{NS} theory.

\section{Cross-Cutting Concerns}
\label{sec:ccc}

Each \acrshort{NS} element consists of business logic encapsulated in a \textit{building block}. The building block executes the business logic code, as well as it provides access to cross-cutting concerns (e.g. persistence, access control).

This very same concept is used by various \textit{microservice ready} frameworks, e.g., Spring Boot\footnote{http://spring.io/projects/spring-boot}. The Spring Boot application out-of-the-box contains some cross-cutting concerns (e.g. security and logging), others can be added via configuration (e.g. Actuator plugin for monitoring) and others can be added simply by including a dependency in a build script (e.g. JPA access).
...
To summarize, the engineering community is dealing with the cross-cutting concerns in a very similar manner, thus it is considered compliant with \acrshort{NS}. (However, there is nothing to learn from \acrshort{NS},.)

\section{Transactional Management}
\label{sec:appl:trans}
\subsection{N-phase Commit}
The 2-phase as well as 3-phase commit is a relationship coordinator -- cohort(s). That means it is orchestrated by a single entity. All communication and coordination is done through single entity and it is the only one keeping the state of the commit (i.e., of a workflow). Therefore, the number of dependencies in the transaction is not rising faster than the number of participants and does not became unbounded. 

The fact that the transaction is blocking/non-blocking does not seem to break any of the rules of evolvability.

Therefore both transaction modes are considered compliant with a \acrshort{NS}.

\subsection{Saga}
Saga has two possible modes of execution:

\subsubsection*{Orchestration}
The orchestration is also a coordinator --- cohort relationship. It exhibits the same properties as the 3-phase commit (it is non-blocking and eventually consistent). As a result, it is considered compliant with the \acrshort{NS} theory.

It is also the preferred way to execute sagas in various literature, e.g., \cite{ms-patterns}.

\subsubsection*{Coordination}

Saga using the \textit{choreography} principle exhibits a risk of tight coupling, since there's no fixed number dependencies on which of the particular participant depends, therefore, the number may eventually become unbounded and complexity rises unnecessarily. Therefore, this way of execution is not compliant with the \acrshort{NS} theory.

This concern is raised in professional literature \cite{ms-patterns}.

\section{Polyglotism and Technological Diversity}

In \acrshort{MSA} the services communicate only via \acrshort{API}s or via message brokers, therefore they can not depend on each other's implementation details. Therefore, the polyglot approach does influence the architectural complexity.

\section{Persistence}
The one DB per \acrshort{MS} pattern prohibits any integration throught database or other code smells, it is perfectly compliant with the \textit{separation of concerns} design theorem.
...
There might be some \textit{guidelines}, \textit{lessons learned} or proofs provided by the \acrshort{NS} theory. For example it proves the high coupling in choreographed saga; encapsulation of bussiness logic code to a framework managing cross-cutting concerns (see \ref{sec:ccc}).
...
Confirmations of such recommendation or obvious facts is not credited to \acrshort{NS} theory.
