...
\todo{rychle dopsat nebo vyhodit}
...
Normalized Systems consider persistence a cross-cutting concern. Due to the \acrshort{MSA} pattern \textit{one DB per service}, it is the private matter of the single service. None of the principles of the \acrshort{NS} seem to contradict that assumption. Therefore, the definition of inner ans outer architecture is left as-is.
...
This very same concept is used by various \textit{microservice ready} frameworks, e.g., Spring Boot\footnote{http://spring.io/projects/spring-boot}. The Spring Boot application out-of-the-box contains some cross-cutting concerns (e.g. security and logging), others can be added via configuration (e.g. Actuator plugin for monitoring) and others can be added simply by including a plugin (e.g. JPA access).
...
To summarize, the engineering community is dealing with the cross-cutting concerns in a very similar manner, thus there is not much to learn from \acrshort{NS}.

\section{Transactional Management}
\label{sec:appl:trans}
\subsection{N-phase Commit}
The 2-phase as well as 3-phase commit is a relationship coordinator --- cohort(s). That means it is orchestrated by a single entity, all communication and coordination is done through this entity and it is the only one keeping the state of the commit (i.e., workflow). This is the very same approach that is used in Normalized Systems.
...
There might be some \textit{guidelines}, \textit{lessons learned} or proves provided by the \acrshort{NS} theory. For example it proves the high coupling in choreographed saga; or it encapsulates bussiness logic code to a framework managing cross-cutting concerns (see \ref{sec:ccc})

On the other hand, this fact is also revealed by microservices literature (although, using just a simple example instead of logical proofs).
