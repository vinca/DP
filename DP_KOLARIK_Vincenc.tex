...
\begin{itemize}
    ...
    \item \textbf{Topology does not change}, and \textbf{the network is homogeneous} \todo{odkazat na service discovery nebo nejak ukecat}
...
\begin{itemize}
    \item[\faCheck] \textbf{Microservice scope} --- a crucial question which is answered only with a very vague guidelines. Will be discussed separately.
    ...
    \item[\faCheck] \textbf{Technological diversity, polyglotism, etc.} --- one of the most hyped benefits of \acrshort{MSA} and potential source of complexity.
    ...
    \item[\faRemove] \textbf{Interaction models and styles} --- only a taxonomy, does not represent real patterns or problems. The real communication complexity is discussed in \fullref{sec:appl:nphase} as well as \fullref{sec:appl:saga}. 
    \item[\faCheck] \textbf{Service contracts} --- \todo{nevim}
    ...
    \item[\faRemove] \textbf{Scalability / Elasticity} --- scalability is matter of operations, therefore does not fall into the scope of this thesis. The only requirements to the  
    \item[\faRemove] \textbf{Availability and Resilience} --- 
...
\begin{itemize}
	...
	\vspace{0.5em}	
	Accomplished --- the compliance with \acrlong{NS} was 

	\item \textit{Summarize design guidelines for \acrshort{MSA}}
	
	\vspace{0.5em}
	Accomplished --- a set of design guidelines was proposed and demonstrated on suitable examples. Since the design guidelines are isolated suggestions, and not a consistent methodology, the originally proposed case study was considered unnecessary. 
